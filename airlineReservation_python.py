...
passengers_df = pd.DataFrame(passengers_full_list[1:], columns=["firstName", "lastName", "address", "age", "origin", "dest",
...
for time in passengers_df.bookingTime:
    ...
    if time[-2] == ":":
        passengers_df.bookingTime[index] = passengers_df.bookingTime[index][:-1] + "0" + passengers_df.bookingTime[index][-1]
    index += 1
...
for time in passengers_df.bookingTime:
    ...
    if time[-5] == ":":
        passengers_df.bookingTime[index] = passengers_df.bookingTime[index][:-4] + "0" + passengers_df.bookingTime[index][-4:]
    index += 1
...
flight_times = ['23:00:00', '22:00:00', '21:00:00', '20:00:00', '19:00:00', '18:00:00', '17:00:00', '16:00:00', '15:00:00',
			...
			if origin_airport != destination_airport:
				...
				for row in reservations_df.itertuples():
					...
					while passenger < reservation_total_pass:
						...
						passengers_seats_list.append((passengerID_dict[(row[1], row[2], row[3], row[4])], len(flights_list), seat))
...
for reservation in reservations_list:
	mycursor.execute("INSERT INTO airline.reservations(passengerID, flightID, originAirport, destinationAirport, departure, class, bookingTime, npass) VALUES(%s, %s, %s, %s, %s, %s, %s, %s)",
...
mycursor.execute("CREATE TABLE seats_on_flights AS SELECT airline.flights.flightID, airline.seats.class, airline.seats.seatNumber FROM flights, seats")
...
# Add Primary Key
mycursor.execute("ALTER TABLE checkIn ADD PRIMARY KEY (flightID, seatNumber, departureTime, originAirport, destinationAirport)")
# Create Index
...
# Sample Passenger to Demonstrate checkIn Function
## Check In Passenger 48 for Flight #2289
checkIn_passenger(48, 2289)